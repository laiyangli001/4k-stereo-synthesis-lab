...
if IS_CUDA:
    ...
    def process(img_uint8: torch.Tensor, target_height: int) -> torch.Tensor:
        ...
        new_height = max(2, (target_height // 2) * 2)
        if new_height == H0:
...
class DepthModelWrapper:
    ...
    def _release_model_instance(self, model):
        if model is None:
            return
        try:
        ...
        try:
            if self._pending_backend == "TensorRT":
                ...
                if engine is not None:
                    ...
                    if prev_model is not None and prev_model is not self.model:
                        self._release_model_instance(prev_model)
                else:
    ...
    def _load_pytorch_model(self, enable_trt=USE_TENSORRT, compile_model=True):
        ...
        if compile_model and self.is_cuda and self.use_torch_compile and not enable_trt:
    ...
    def _load_coreml_model(self, engine_h, engine_w):
        ...
        if not os.path.exists(COREML_PATH) or RECOMPILE_COREML:
            pytorch_model = self._load_pytorch_model(enable_trt=False, compile_model=False)
            try:
                export_to_coreml(
                    pytorch_model,
                    COREML_PATH,
                ...
                self._release_model_instance(pytorch_model)
    ...
    def close(self):
        ...
        if model is None:
            return
        self._release_model_instance(model)
        try:
            del self.model
        except Exception:
            pass
...
def ensure_inference_runtime_loaded():
    global model_wraper, MODEL_DTYPE, _engine_ready
    with _runtime_lock:
        if not _INFERENCE_ACTIVE:
            return False
        if model_wraper is not None:
            return True
        model_wraper = _build_model_wrapper()
        MODEL_DTYPE = next(model_wraper.model.parameters()).dtype if hasattr(model_wraper.model, 'parameters') else DTYPE
        _rebuild_norm_tensors()
        _engine_ready = False
        if 'depth_stabilizer' in globals():
            depth_stabilizer.prev = None
        print("[Depth] Inference runtime loaded")
        return True


def unload_inference_runtime():
    global model_wraper, MODEL_DTYPE, MEAN, STD, _engine_ready, ENGINE_H, ENGINE_W
    unloaded = False
    with _runtime_lock:
        if model_wraper is not None:
            try:
    ...
    gc.collect()
    if unloaded:
        print("[Depth] Inference runtime unloaded")


def set_inference_active(active: bool):
    global _INFERENCE_ACTIVE
    active = bool(active)
    if _INFERENCE_ACTIVE == active:
        return
    _INFERENCE_ACTIVE = active
    if active:
        return
    unload_inference_runtime()


ensure_inference_runtime_loaded()
...
def predict_depth(image_rgb, return_tuple=False, use_temporal_smooth: bool = True, dtype=DTYPE):
    ...
    """
    if not ensure_inference_runtime_loaded():
        return None
