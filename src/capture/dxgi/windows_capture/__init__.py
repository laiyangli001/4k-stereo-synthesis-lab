...
class DxgiDuplicationGpuFrame:
    """Represents a GPU-backed DXGI desktop duplication frame."""

    __slots__ = ("_native",)

    def __init__(self, native_frame: NativeDxgiDuplicationGpuFrame) -> None:
        self._native = native_frame
    ...
    @property
    def texture_ptr(self) -> int:
        """ID3D11Texture2D pointer for the acquired duplication texture."""
        return int(self._native.texture_ptr())

    @property
    def shared_texture_ptr(self) -> int:
        """ID3D11Texture2D pointer for the shared GPU copy, or 0 if not requested."""
        return int(self._native.shared_texture_ptr())

    @property
    def shared_handle(self) -> int:
        """DXGI shared texture handle, or 0 if not requested."""
        return int(self._native.shared_handle())

    @property
    def has_shared_handle(self) -> bool:
        return bool(self._native.has_shared_handle())
...
class DxgiDuplicationSession:
    ...
    def acquire_gpu_frame(self, timeout_ms: int = 16, shared: bool = True) -> Optional[DxgiDuplicationGpuFrame]:
        native_frame = self._native.acquire_next_gpu_frame(timeout_ms, shared)
        if native_frame is None:
            return None

        return DxgiDuplicationGpuFrame(native_frame)
