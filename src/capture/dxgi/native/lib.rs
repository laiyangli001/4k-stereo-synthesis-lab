...
use windows::Win32::Graphics::Direct3D11::{
    ...
    D3D11_RESOURCE_MISC_SHARED, D3D11_TEXTURE2D_DESC, D3D11_USAGE_DEFAULT, D3D11_USAGE_STAGING, ID3D11Texture2D,
};
...
fn windows_capture(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_class::<NativeDxgiDuplicationGpuFrame>()?;
    Ok(())
}
...
impl NativeDxgiDuplication {
    ...
    #[pyo3(signature = (timeout_ms=16, shared=true))]
    pub fn acquire_next_gpu_frame(
        &mut self,
        timeout_ms: u32,
        shared: bool,
    ) -> PyResult<Option<NativeDxgiDuplicationGpuFrame>> {
        match self.duplication.acquire_next_frame(timeout_ms) {
            Ok(frame) => {
                let texture_desc = *frame.texture_desc();
                let color_format = Self::color_format_from_dxgi(texture_desc.Format)?;
                let source_texture = frame.texture().clone();
                ...
                if shared {
                    ...
                    unsafe { device.CreateTexture2D(&shared_desc, None, Some(&mut texture)) }
                        .map_err(|e| PyException::new_err(format!("Failed to create shared DXGI texture: {e}")))?;
                    let texture = texture.expect("CreateTexture2D returned Ok but no texture");

                    unsafe {
                        device_context.CopyResource(&texture, frame.texture());
                    }

                    let dxgi_resource: IDXGIResource = texture.cast().map_err(|e| {
                        PyException::new_err(format!("Failed to cast shared texture to IDXGIResource: {e}"))
                    })?;
                    let handle = unsafe { dxgi_resource.GetSharedHandle() }
                        .map_err(|e| PyException::new_err(format!("Failed to get shared DXGI texture handle: {e}")))?;

                    shared_handle = handle.0 as usize;
                    shared_texture = Some(texture);
                }

                Ok(Some(NativeDxgiDuplicationGpuFrame::new(
                    source_texture,
                    shared_texture,
                    shared_handle,
                    texture_desc.Width,
                    texture_desc.Height,
                    Self::bytes_per_pixel(color_format),
                    Self::color_format_to_str(color_format),
                )))
            }
            Err(DxgiDuplicationError::Timeout) => Ok(None),
            ...
            Err(other) => Err(PyException::new_err(format!("Failed to acquire duplication GPU frame: {other}"))),
        }
    }
...
#[pyclass(unsendable)]
pub struct NativeDxgiDuplicationGpuFrame {
...
impl NativeDxgiDuplicationGpuFrame {
    fn new(
        texture: ID3D11Texture2D,
        shared_texture: Option<ID3D11Texture2D>,
        shared_handle: usize,
        ...
        Self { texture, shared_texture, shared_handle, width, height, bytes_per_pixel, color_format }
    }
}
...
impl NativeDxgiDuplicationGpuFrame {
    ...
    pub fn shared_texture_ptr(&self) -> usize {
        self.shared_texture.as_ref().map_or(0, |texture| texture.as_raw() as usize)
    }

    pub fn shared_handle(&self) -> usize {
        self.shared_handle
    }

    pub fn has_shared_handle(&self) -> bool {
