...
MESSAGE_CATALOGS = {
    "EN": {
       ...
       "Preview":"Preview",
        ...
        "tooltip_depth_strength": "Overall stereo depth intensity. Use Standard / 2.5 as the baseline; raise it only when the scene feels flat, and lower it when foreground objects show ghosts, edges tear, or viewing feels uncomfortable.",
        ...
        "tooltip_render_policy": "Runtime render-size policy. Native keeps the captured size, Scaled applies Render Scale, Fixed uses Fixed Size, and Dynamic clamps by pixel cap and minimum side.",
        "tooltip_render_scale": "Scale factor used when Render Policy is Scaled.",
    ...
    "CN": {
        ...
        "tooltip_depth_strength": "整体立体深度强度。建议以标准档 2.5 为基准；画面太平时再上调，前景重影、边缘撕裂或观看不舒服时下调。",
        "tooltip_depth_quick": "给普通用户使用的固定深度档位：柔和、标准、增强",
        "tooltip_stereo_preset": "立体预设模式。传统速度最快，电影使用 quality_4k，游戏使用 fast_plus，图片使用 hq_4k；选择模式会加载对应高级参数。",
        ...
        "tooltip_render_policy": "运行时渲染尺寸策略。原生保持捕获尺寸，缩放使用渲染缩放，固定使用固定尺寸，动态按像素上限和最短边约束。",
        "tooltip_render_scale": "渲染策略为缩放时使用的尺寸倍率。",
