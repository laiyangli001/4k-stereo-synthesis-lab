...
12. Panda3D glTF renderer 是 `docs/39` 定义的成熟 runtime 迁移候选：它只能作为 opt-in 路径接入现有 OpenXR D3D11 Projection swapchain，不能替换 depth/runtime/latest-frame 调度、OpenXR session、controller input 或屏幕拖动状态机；默认 renderer 在真机 gate 通过前保持 native/OpenGL 测试路径。
```
...
| Debug / result contract | `StereoRuntimeResult` / `OpenXRRuntimeResult` 已暴露 output/timing/provider 结构化字段；每帧 debug_info 已补齐 application_runtime_target、stereo_synthesis_mode、transport、output_transport、capture/render/depth size、active settings metadata、dynamic convergence、Depth Pop 和 FG/MG/BG Pop 诊断字段；OpenXR async 增加 `validate_openxr_async()`、`screen_proj` 和 `openxr_async_ok/missing/failed` 日志验收摘要；OpenXR loop segment 性能日志限制为最多 5 次，Panda bridge 成功诊断日志也限制为最多 5 次，并输出 `_panda_render_success_count`、`_panda_render_failure_count`、`_panda_render_last_bridge_mode`、`_panda_render_last_target_size`、`_panda_render_last_image_indices`、`_panda_render_error`、`_panda_render_last_timing` | debug-only 兼容键仍按兼容清理表逐步移除；host/viewer 新消费路径应继续优先读结构化字段；OpenXR 完成判定必须以实机日志显示 `screen_proj>0`、`openxr_async_ok=1` 且 missing/failed 均为 none 为依据；Panda3D migration 完成判定还必须看到 Panda bridge success > 0、failure == 0、bridge mode 为 `nv_dx`、错误为空、左右眼尺寸/image index 正常且头显中真实可见 |
...
3. 执行 `docs/39` Panda3D glTF OpenXR D3D11 migration gate：保持 default native/OpenGL 测试路径不变，显式启用 `D2S_GLTF_RENDERER=panda3d` 后在真实头显/OpenXR runtime 下验证 Panda offscreen -> NV_DX -> D3D11 Projection swapchain 可见、姿态正确、无同步/显存错误；只有 gate 通过后才考虑把 Panda3D 设为默认或删除 self-authored renderer 分支。
...
6. OpenGL 作为本地 viewer、当前测试默认和兼容 fallback：继续验证 `GlTensorPboUploader` / `CudaGlTextureUploader` 的 image/PBO/CPU fallback 日志，不再把 OpenGL uploader 当作 D3D11 方案；Panda3D 自身固定使用 OpenGL offscreen，但它通过 NV_DX bridge 交给 D3D11 OpenXR swapchain，不等同于 OpenXR 主 backend 切回 OpenGL。
