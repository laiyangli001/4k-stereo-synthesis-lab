...
- Panda3D migration is an opt-in path, not the default renderer. Current code has Panda3D asset probes, node-animation sampling, model-layer frame-state adapters, Panda offscreen targets, NV_DX bridge plumbing, OpenGL fallback bridge plumbing, and Panda projection bridge diagnostics. Panda runtime is now explicitly limited to environment/controller GLB, animation, eye views/FOV, and controller poses; screen texture, screen pose, and controller ray classes/bindings were removed so virtual screen, laser, keyboard, FPS/OSD, and final composition stay in the existing OpenXR compositor. The hard gate remains real headset/OpenXR runtime proof that Panda3D can render into the currently acquired OpenXR projection swapchain texture with correct eye pose, no sync/device errors, no CPU/PBO/PIL/Numpy readback, and no fallback to native. Ordinary D3D11 texture interop success is only readiness; it is not proof that real OpenXR swapchain acquire/render/release has passed.
...
- `docs/39` now states the hard GPU-only/zero-copy requirement explicitly. Panda3D may own glTF scene graph, animation, materials, environment/controller model rendering, and per-eye cameras, but it must not own virtual screen, controller laser/ray, keyboard, FPS/OSD, acquire/wait/release/submit, or projection swapchain lifecycle.
...
- Added Windows WGL sharing before Panda eye texture creation: the current GLFW/OpenXR `HGLRC` is shared with the newly created Panda `ShowBase` `HGLRC` through `wglShareLists`, then the code returns to the GLFW/OpenXR context for the visibility gate. The local context-pair probe succeeds; real OpenXR swapchain proof remains pending.
...
- Added a pre-acquire OpenGL visibility gate: after Panda target prewarm and an explicit switch back to the OpenXR GL context, Panda target texture ids must be visible via `glIsTexture`; otherwise the Panda OpenGL bridge is disabled before any swapchain acquire.
- First real OpenGL Panda bridge run proved WGL sharing and acquire/render/release (`success_count=5`, `failure_count=0`) but showed black output and Panda `GL 0x502`. The cause was rendering Panda while the OpenXR context remained current. Targets now retain Panda HDC/HGLRC, switch to it before every Panda `render_frame()`, and return to the OpenXR context only for the GPU blit.
- Second real OpenGL run removed the GL errors and kept bridge success/failure at `5/0`, but the headset showed only Panda's gray clear color. The bridge was valid; geometry was outside the camera because OpenXR/OpenGL poses were copied directly into Panda coordinates, the environment profile model transform was omitted, and the adapter attempted to call a nonexistent viewer `_view_pose_mat4`, leaving both Panda eye poses unset. Frame-state conversion now maps OpenXR `(x,y,z)` to Panda `(x,-z,y)` with basis-converted rotation, Panda environment roots consume the existing model position/rotation/scale profile, and real `XrView.pose` values fall back through `xr_pose_to_model_mat4`. The startup diagnostic now reports `eye_poses=2` only when both real camera poses are present. The next real-headset run confirmed the room and controller models are visible, narrowing the remaining missing image to the Artemis skybox. The old GLB exported that node as `alphaMode=MASK`, outward-facing one-sided geometry, so it depended on native/profile overrides. The Unity exporter now writes inward-facing winding/normals with an `OPAQUE` `KHR_materials_unlit` material, and the re-exported skybox became visible in-headset. The Panda-only `configure_environment_skybox` facade, profile node matching, alpha/cull/background/depth overrides, result fields, and diagnostics have now been deleted. Static validation confirms the current GLB is `OPAQUE`, one-sided and unlit, while direct `panda3d-gltf` loading adds no special skybox RenderState. The no-override headset retest displayed the skybox but exposed intermittent uniform-gray polygon gaps. Pixel and mesh diagnostics proved these were Panda clear color revealed by far-plane clipping, not GL errors or mesh holes: the closed inward sky sphere reaches about 1033.5 units from the configured view while Panda Lens was fixed at far=1000. Panda frame state now carries the existing viewer/profile near/far values, both eye lenses update them every frame, and the startup log reports `clip=near/far`; Artemis should report `clip=0.100/20000.0`. A headset retest is required to confirm the gaps are gone.
...
- Corrected the Unity export contract in `H:\My project\Assets\LoadAndExport.cs`: baked and no-bake exports now apply one root-level inverse X/Z scene-bounds-center transform, preserving Unity Y/floor height while making the effective GLB horizontal center the OpenXR origin. The same root transform carries meshes, skybox, animation nodes, cameras, and lights.
- Tightened Unity material export classification. Skybox, explicit emissive objects, and generated baked atlas/detail materials use unlit; ordinary room/ground/station/chair/decorative/metal/transparent materials use PBR. A stale `_EMISSION` keyword alone no longer marks a material emissive, and no-bake now uses the same normalized material conversion as the baked hybrid path.
- The re-exported Artemis GLB now has root translation `X=4629.44336`; transformed non-sky bounds are horizontally centered near `(0, 0)`. The matching profile migration is applied: historical `model_position.x=4629.4438` is now `0.0`, and `model_position.y` is restored from the incorrect `985.6101` to the previously validated `843.0854`; Z, `view_poses`, screen layout, rotation, and scale remain unchanged. Static verification now places the skybox at `Y=0..1800`, ground at `Y=888.77..906.38`, chairs at `Y=899.35..905.98`, station at `Y=888.46..963.53`, and the active preview seat at `Y=909.06`, about 3.08 units above the chair top. Material validation still fails until the newest exporter output is rechecked: the last inspected GLB remained 44/44 unlit and 0 PBR. The Unity exporter now uses a stricter semantic emissive whitelist and ordinary-room-surface-first PBR rule; another re-export is required before material and headset acceptance.
- Fixed `preview_room_layout.py` profile reload semantics: `R` now rebuilds `env_model` from the reloaded model position/rotation/scale, recomputes transformed bounds, refreshes navigation speed, and reloads profile projection near/far. Preview rendering now uses `xr_projection_near/xr_projection_far` instead of the old fixed `far=200`; Artemis reports `Preview projection: clip=0.100/20000.0`, so its approximately 900-unit-radius sky sphere is not clipped. Previously a running window kept stale model/projection state and could save it back over the corrected profile. Existing preview processes must be closed and restarted once after this code change.
- Connected profile lighting to the Panda model-layer renderer. `sync_panda_scene_assets_from_viewer()` now reads viewer ambient/head/fill light profile state, includes it in the scene binding key, and calls the Panda facade lighting configurator. `PandaSceneGraph` owns the Panda light lifecycle, installs lights on the next Panda render-base attach, and updates the head light from the current eye midpoint. Day/Night preset changes therefore rebuild Panda lights on the next frame without reloading the environment asset.
- Fixed the Unity exporter follow-up after the first PBR re-export produced `baked meshes=0, hybrid PBR meshes=44` but still wrote `KHR_materials_unlit` on 44/44 materials. The root cause was second-stage material conversion: source hierarchy/shader semantics still looked unlit/emissive, and `ShouldPreserveOriginalTexture()` treated chair/seat as detail-preserve unlit. `LoadAndExport.cs` now lets ordinary material names such as chair/ground/station override hierarchy screen/emissive noise and prevents ordinary room surfaces from using the detail/unlit preserve branch. Re-export is required before static GLB validation can pass.
- The latest Artemis re-export statically validates the intended material split: 44 materials total, 29 `KHR_materials_unlit` materials for satellites/spaceships/line effects/explicit unlit colors/skybox, and 15 ordinary room PBR materials for chairs, ground, and station (`*_gltf_standard`). This clears the static material gate; the next required step is real-headset validation of Panda profile lighting brightness and color.
- The first headset run after the material split showed correct models/textures but the Panda scene remained too dark. Logs confirmed bridge success and `lights=ambient/head/fill` active, so the issue is lighting calibration rather than GLB material fallback. Added Artemis `panda_light_scale=4.0` and taught Panda scene binding to scale only Panda ambient/head/fill colors from profile keys (`panda_light_scale`, `panda3d_light_scale`, or `env_panda_light_scale`). Native/preview profile lighting semantics are unchanged. Binding logs now print the actual scaled ambient/head values; Artemis Day should show approximately `ambient:0.24,0.2,0.2` and `head:1.8,1.8,1.92`.
- The next headset screenshots showed the same GLB looks normal in preview and external glTF viewers but too dark in the Panda OpenXR path, with the skybox effectively disappearing. The likely gap is Panda offscreen color-space output, not a need to restore profile skybox node overrides. Panda stereo targets now request `framebuffer-srgb true`, use `Texture.F_srgb_alpha`, set `FrameBufferProperties.set_srgb_color(True)`, and enable `GL_FRAMEBUFFER_SRGB` only during Panda `render_frame()` before blitting encoded color to the OpenXR sRGB swapchain. This preserves the current rule that skybox visibility must come from the standard GLB asset (`OPAQUE` + `KHR_materials_unlit` + inward geometry), not from Panda node-name special handling. Real-headset validation is required to confirm brightness, chair material appearance, and skybox visibility.
...
.\src\python3\python.exe -m pytest tests\test_openxr_runtime.py tests\test_panda_runtime_adapter.py -q -p no:cacheprovider
```
...
Panda/OpenXR regression suite passed: 171 passed, 2 warnings
Unity `Assembly-CSharp` Bee response-file compile passed; only Unity source-generator version warnings were emitted.
```

Remaining validation target:

- Re-export Artemis again after the strict semantic emissive-whitelist fix. Static validation must keep the transformed non-sky horizontal center near `(0, 0)` and show ordinary room materials as PBR, while only skybox, explicit emissive, and generated baked materials remain unlit.
- The profile coordinate migration is already complete; do not reapply the old `4629.4438` X compensation or the incorrect `985.6101` Y offset. Keep `model_position=[0.0, 843.0854, -0.1649]`, then use `preview_room_layout.py` only for intentional seat/screen fine tuning after the new material export passes static validation.
...
- Added the `src/xr_viewer/panda_runtime/` adapter layer: `PandaSceneRenderer`, environment/controller scene ownership, stereo target specs, model-layer frame-state snapshots, animation clock, diagnostics, and bridge contracts. Screen texture target and controller ray target were removed from the runtime contract.
...
- Wired Panda frame-state update after `xrLocateViews` so the same frame snapshot carries only eye views, controller poses, and animation time. Controller rays, screen pose, and screen texture stay in the existing OpenXR compositor path, not Panda runtime.
