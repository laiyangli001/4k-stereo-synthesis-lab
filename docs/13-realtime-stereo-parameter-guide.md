...
| `Anti-aliasing` / `Depth Antialias Strength` | GUI `0-10`；默认 `2` / `0.4` | 平滑深度图，减少硬边和噪声。 | 增大后边缘更稳、闪烁少，但深度边界会糊，前景轮廓可能不准。 | 实时 `0-1`；电影 `1-2`；静态图可更高，但要看边界是否发糊。 |

### 合成质量与速度参数

| 参数 | 当前范围 / 默认 | 原理 | 观察现象 | 调参建议 |
|---|---|---|---|---|
| `Stereo Preset` | `auto / cinema / game_low_latency / still_image_hq / debug_export`；默认 `auto` | 一组参数预设；`auto` 会根据输入、延迟、运动信号切换 preset。 | 控制台会打印当前 preset。auto 会自动切模式，固定 preset 更容易调参。 | 调参时先固定 `cinema` 或 `game_low_latency`，不要用 auto。 |
