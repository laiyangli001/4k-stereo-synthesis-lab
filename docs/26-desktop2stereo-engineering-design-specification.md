...
| 步骤 2：Resolve Render Size | `src/stereo_runtime/render_size.py`, `src/stereo_runtime/pipeline.py`, GUI Render Scale 配置 | 已按 4K/3K/2K/1K 固定档位解析；历史 numeric scale / threshold 兼容属于清理项 |
...
| 步骤 11：Output Pack / Viewer Upload | `src/stereo_runtime/output.py`, `output_triton.py`, `src/viewer/*`, `src/xr_viewer/*`, `src/streaming/*` | 本地/网络消费 packed output，OpenXR 支持 full synthesis 与 RGB+depth direct；D3D11 direct parity 和 RTMP/低延迟 stream 是后续工程 |
...
| render-size/render-scale 旧路径 | 用户侧只保留固定分辨率档位语义 | 删除历史 numeric scale thresholds、`native/fixed/dynamic` 用户策略语言、非 4K 连续缩放行为 |
...
| Render Size / 4K tier | 已收敛为固定档位 Render Scale：非 4K 保持 capture_size；4K 级输入按 4K/3K/2K/1K 稳定档位解析；判断覆盖横屏、竖屏、16:10、DCI 4K、常见 4K 超宽，并排除面积不足的窄高/1440p 超宽 | 清理历史 numeric scale / threshold 兼容冗余；该项属于 Future Work |
